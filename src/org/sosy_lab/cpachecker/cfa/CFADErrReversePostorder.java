...
import org.sosy_lab.cpachecker.cfa.model.DErrComparator;
import org.sosy_lab.cpachecker.util.CFAUtils;

public class CFADErrReversePostorder {
  ...
  public void assignSorting(final CFANode start) {
    ...
    while (!nodeStack.isEmpty()) {
      ...
      if (successors == null) {
        ...
        // enter the for loop
        PriorityQueue<CFANode> sortedset = new PriorityQueue<>(new DErrComparator());
        Iterator<CFAEdge> tsuccessors = CFAUtils.allLeavingEdges(node).iterator();
        while (tsuccessors.hasNext()) {
          CFAEdge tnext = tsuccessors.next();
          CFAEdgeType nexttype = tnext.getEdgeType();
          if (!(nexttype == CFAEdgeType.FunctionCallEdge
              || nexttype == CFAEdgeType.FunctionReturnEdge)) {
            sortedset.add(tnext.getSuccessor());
          }
        }
        successors = sortedset.iterator();
        iteratorStack.pop();
